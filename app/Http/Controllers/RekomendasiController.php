...
class RekomendasiController extends Controller
{
    ...
    public function index(PresetPreference $presetpreference, Request $request)
    {
        ...
        // Ambil bobot AHP atau langsung (seperti kode sebelumnya)
        $bobot = Bobot::where('id_perhitungan', $this_user->id_perhitungan_aktif)->first();
        if (!$bobot) {
            return redirect()->back()->with('error', 'Bobot AHP belum tersedia.');
        }

        // Ambil semua data kandidat
        $kandidats = Pegawai::all();

        // Hitung skor rekomendasi tiap kandidat
        $ranking = $kandidats->map(function ($kandidat) use ($bobot) {
            $skor =
                ($kandidat->pendidikan * $bobot->c1) +
                ($kandidat->pengalaman_kerja * $bobot->c2) +
                ($kandidat->wawancara * $bobot->c3) +
                ($kandidat->sertifikasi_pendukung * $bobot->c4) +
                ($kandidat->kemampuan * $bobot->c5); // tambahkan sesuai jumlah kriteria
        ...
        $ranking = $ranking->sortByDesc('skor');

        return view('rekomendasi.index', compact('ranking'));
    }
